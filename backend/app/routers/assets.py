...
async def _get_asset_or_404(asset_id: int, user: User, session: AsyncSession) -> Asset:
    asset = (
...
async def create_asset(
    ...
    asset = Asset(**payload.model_dump(), user_id=current_user.id)
    session.add(asset)
...
async def get_asset(
    ...
    return await _get_asset_or_404(asset_id, current_user, session)
...
async def update_asset(
    ...
    asset = await _get_asset_or_404(asset_id, current_user, session)
