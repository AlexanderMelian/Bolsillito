...
async def test_list_accounts_excludes_archived_by_default(client, db_session):
    visible = Account(name="Visible", type=AccountType.BANK)
    archived = Account(name="Archivada", type=AccountType.BANK, is_archived=True)
...
async def test_list_accounts_can_include_archived(client, db_session):
    archived = Account(name="Archivada 2", type=AccountType.BANK, is_archived=True)
...
async def test_get_account_returns_existing(client, db_session):
    account = Account(name="Cuenta X", type=AccountType.WALLET)
...
async def test_update_account_partial_patch(client, db_session):
    account = Account(name="Original", type=AccountType.BANK, balance=Decimal("10.00"))
    db_session.add(account)
    await db_session.commit()
...
async def test_delete_account_hard_deletes_when_no_dependents(client, db_session):
    account = Account(name="Sin dependencias", type=AccountType.CASH)
    db_session.add(account)
    await db_session.commit()
...
async def test_delete_account_soft_deletes_when_it_has_a_card(client, db_session):
    account = Account(name="Con tarjeta", type=AccountType.BANK)
    db_session.add(account)
...
async def test_delete_account_soft_deletes_when_it_has_a_transaction(client, db_session):
    account = Account(name="Con movimientos", type=AccountType.BANK)
    db_session.add(account)
    await db_session.flush()
    db_session.add(
        Transaction(
