...
async def test_summary_totals_balances_in_the_same_currency(client, db_session):
    await _account(db_session, name="A", balance=Decimal("1000.00"))
    await _account(db_session, name="B", balance=Decimal("500.00"))
...
async def test_summary_converts_foreign_currency_balance_when_rate_available(client, db_session):
    await _account(db_session, name="ARS", balance=Decimal("1000.00"), currency="ARS")
    await _account(db_session, name="USD", balance=Decimal("10.00"), currency="USD")
    await client.post(
...
async def test_summary_reports_unconverted_balance_without_a_rate(client, db_session):
    await _account(db_session, name="USD", balance=Decimal("10.00"), currency="USD")
...
async def test_summary_sums_income_and_expenses_for_the_month(client, db_session):
...
async def test_summary_excludes_statement_payment_from_month_expenses(client, db_session):
...
async def test_summary_accepts_explicit_month_param(client, db_session):
    account = await _account(db_session)
    other_month = date(TODAY.year, TODAY.month, 1) - timedelta(days=45)
    db_session.add(
        Transaction(
            type=TransactionType.INCOME, account_id=account.id, amount=Decimal("77.00"), date=other_month
        )
    )
    await db_session.commit()
...
async def test_summary_converts_using_the_inverse_rate_when_only_that_is_loaded(client, db_session):
    await _account(db_session, name="USD", balance=Decimal("10.00"), currency="USD")
...
async def test_spending_by_category_groups_and_sums(client, db_session):
    account = await _account(db_session)
    category = Category(name="Comida", kind=TransactionType.EXPENSE, icon="🍔")
...
async def test_spending_by_category_skips_amounts_without_an_exchange_rate(client, db_session):
    account = await _account(db_session, currency="USD")
    db_session.add(
        Transaction(
...
async def test_cash_flow_projection_groups_by_payment_due_month(client, db_session):
    account = await _account(db_session)
    card = await _credit_card(db_session, account)
...
async def test_cash_flow_projection_includes_zero_months(client, db_session):
...
async def test_cash_flow_projection_excludes_paid_statements(client, db_session):
    account = await _account(db_session)
    card = await _credit_card(db_session, account)
    statement = CardStatement(
        card_id=card.id, closing_date=TODAY, payment_due_date=TODAY + timedelta(days=5)
    )
    db_session.add(statement)
    await db_session.flush()
    db_session.add(
...
async def test_cash_flow_projection_skips_empty_statements(client, db_session):
    account = await _account(db_session)
    card = await _credit_card(db_session, account)
    db_session.add(
        CardStatement(
...
async def test_cash_flow_projection_skips_amounts_without_an_exchange_rate(client, db_session):
    account = await _account(db_session, currency="USD")
    card = await _credit_card(db_session, account)
    statement = CardStatement(
        card_id=card.id,
